...
public class CustomerValidatorImpl implements CustomerValidator{
    ...
    private void validatePayMethodsDTOList(List<CustomerPayMethodDTO> customerPayMethodDTOS, Long id, List<BodyReport> reports){
        for(int i = 0; i< customerPayMethodDTOS.size(); i++){
            if (customerPayMethodDTOS.get(i).getCustomerId()==null) reports.add(new BodyReport("e-001", "payMethods["+i+"]", "payMethods["+i+"].customerId must be not null"));
            else if (!customerPayMethodDTOS.get(i).getCustomerId().equals(id)) reports.add(new BodyReport("e-002", "payMethods["+i+"]", "payMethods["+i+"].customerId incorrect. It must equals customer.id"));
        }
    }
    ...
    private void validateAddressDTOList(List<AddressDTO> addressDTOS, Long id, List<BodyReport> reports){
        for(int i=0; i<addressDTOS.size(); i++){
            if (addressDTOS.get(i).getCustomerId()==null) reports.add(new BodyReport("e-001", "address["+i+"]", "address["+i+"].customerId must be not null"));
            else if (!addressDTOS.get(i).getCustomerId().equals(id)) reports.add(new BodyReport("e-002", "address["+i+"]", "address["+i+"].customerId incorrect. It must equals customer.id"));
        }
    }

    private void validateString(String str, Pattern pattern, String fieldName, boolean isUnique, List<BodyReport> reports){
        if (str==null || str.isBlank()) reports.add(new BodyReport("e-001", fieldName, fieldName+" must be not empty (only whitespaces / null)"));
        if (!pattern.matcher(str).matches()) reports.add(new BodyReport("e-002", fieldName, "Incorrect "+fieldName+" format"));
    ...
    private void checkIsUniqueValueExistsByFieldName(String str, String fieldName,List<BodyReport> reports){
        //get all methods and find all is*fieldName*Exists?
        ...
        if (method!=null){
            try {
                if((Boolean)method.invoke(customerService,str)) reports.add(new BodyReport("e-003", fieldName, fieldName+" is already exists. This parameter should be unique"));
    ...
    private void validateDob(String dob, List<BodyReport> reports){
        if (dob==null || dob.isBlank()) reports.add(new BodyReport("e-001", "dob", "Dob must be not empty (only whitespaces / null)"));
        LocalDate date= LocalDate.parse(dob);
        if (!date.isAfter(LocalDate.now().minusYears(OLD_LIMIT))) reports.add(new BodyReport("t-001", "dob", "Incorrect dob. Customer must be younger "+OLD_LIMIT));
        if (!date.isBefore(LocalDate.now().minusYears(YOUNG_LIMIT))) reports.add(new BodyReport("t-002", "dob", "Incorrect dob. Customer must be older "+YOUNG_LIMIT));
    }
}
